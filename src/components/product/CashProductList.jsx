			...
			<div className={productStyle.itemWrap} style={{marginBottom:'0.24rem'}}>
              ...
              {data.commFlag === '1'?
                <p>
                  ...
                  {data.commUseableAmt.indexOf('限额') !== -1 ?
                    <span className={productStyle.txt4}><i>该产品限额,无实际可约额度</i></span> :
                    <span className={productStyle.txt4}><i>系列可用额度(万)&nbsp;</i>{data.commUseableAmt !== undefined ? data.commUseableAmt : '--'}</span>}
                </p>
                :
                <p>
                  <span className={productStyle.txt4}><i>可用合同数&nbsp;</i>{data.useableCont !== undefined ? data.useableCont : '--'}</span>
		...
		return (
                ...
                </section>
                {
                  <p className={productStyle.orderBtn}><span>预约</span></p>
								...
								<div className={productStyle.trendDet}>
									<p>
										<span className={productStyle.txt2}>{data.earnType==1?data.profRuleShortDesc:data.netVal}</span>
										<span className={productStyle.margin0}>&nbsp;</span>
										<span className={productStyle.margin0}>{data.earnType==1?`预计收益率`:`净值`}</span>
									</p>
									<p>
...
export default CashProductList;
